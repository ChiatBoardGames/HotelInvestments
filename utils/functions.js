...
export function getStockInfo(id, tileCount) {
...
export function rewardBonuses(players, chain, majorityBonus, minorityBonus) {
...
export function findNeighbors(board, tile) {
...
export function placeTile(board, tile, value) {
  return board.setIn(tile, value);
}

export function placeTiles(board, tiles, value) {
  return tiles.reduce((board, tile)=> {
    return placeTile(board, tile, value);
  }, board);
}

export function nextActivePlayer(state, game) {
  return state.update('activePlayer', ap=> (ap + 1) % game.players.length);
}

export function nextActivePlayerForMerging(state, game) {
...
export function getNextPlayerForMerging(state, game, atLeastOnce) {
