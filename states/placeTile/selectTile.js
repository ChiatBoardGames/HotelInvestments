...
export default function selectTileFactory({Immutable, Error, logger, functions, _}) {
  const {
    findNeighbors,
    placeTile,
    placeTiles,
  } = functions;
  return function selectTile(game, playerId, {tile}) {
    game = Immutable(game);
    ...
    if(neighbors.length === 0 && !emptyNeighbor) {
      ...
      if(game.chains && game.chains.length >= 7) {
